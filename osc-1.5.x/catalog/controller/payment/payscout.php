...
class ControllerPaymentPayscout extends Controller {
	...
	public function send() {
		...
		if($this->config->get('payscout_server') == 0)
		{
		...
		{
			$json['error']['cc_billing_ssn'] = 'Please enter 4 digit SSN Number.';
			$error++;
		...
		if(!isset($_POST['cc_dob']))
		{
			$json['error']['dob'] = 'Please select date of birth.';
			$error++;
		}else if(isset($_POST['cc_dob']) && trim($_POST['cc_dob'])=="")
		{
			$json['error']['dob'] = 'Please select date of birth.';
			$error++;
		}
		...
		{
			$json['error']['cc_number'] = 'Please enter card number.';
			$error++;
		...
		{
			$json['error']['cc_cvv2'] = 'Please enter cvv number.';
			$error++;
		...
		$request = array(			
			...
			'billing_date_of_birth' => date('Ymd', strtotime($this->request->post['cc_dob'])),
